...
const DealCard = ({ name, description, price, imageUrl, _id , likes, created_at }) => {
  ...
  return (
      <>
<CardGroup>
  <Card className="cardSpacing" border="warning" style={{ width: '14rem' }}>
    <Card.Img variant="top" className= "imgDeals rounded" src={`${imageUrl}`} />
    <Card.Body>
      <Card.Title >{name.length < 35
                ? `${name}`
                : `${name.substring(0, 35)}...`}</Card.Title>
    ...
    <Row>
    <Col>
    {likes&&likes.length}<Image className="icons" src="../../../../icons/fire.png"></Image>
    4<Image className="icons" src="../../../../icons/icicle.png"></Image>
                    </Col>
    <Col>  

        <Image className="icons" src="../../../../icons/alarm-24px.svg"></Image>
      <small className="text-muted">{moment(created_at).subtract(6, 'days').calendar()}</small>
